// https://raw.githubusercontent.com/kurgm/kage-editor/master/src/kageUtils/connection.ts
// author: kurgm
...
const matchConnectablePoint = (
  filter: ConnectablePointFilter,
  point: ConnectablePoint
) =>
...
const isConnectable = (point1: ConnectablePoint, point2: ConnectablePoint) =>
  ...
  connectablePairs.some(
    ([filter1, filter2]) =>
      (matchConnectablePoint(filter1, point1) &&
        matchConnectablePoint(filter2, point2)) ||
      (matchConnectablePoint(filter2, point1) &&
        matchConnectablePoint(filter1, point2))
  );
  ...
  switch (glyphLine.value[0]) {
    ...
    case 7: {
      result.push({
        ...
        coord: [glyphLine.value[3], glyphLine.value[4]],
        pointIndex: 0,
      });
...
export const listupConnectedPoints = memoizeOne(
  (glyph: Glyph, points: PointDescriptor[]): PointDescriptor[] => {
...
export const listupConnectedPointsOfSelection = memoizeOne(
  (glyph: Glyph, selection: number[]): PointDescriptor[] => {
    ...
    for (const lineIndex of selection) {
      ...
      selectedDescs.push({
        lineIndex,
        pointIndex: (getNumColumns(glyphLine.value[0]) - 3) / 2 - 1,
      });
    }
    return listupConnectedPoints(glyph, selectedDescs).filter(
      ({ lineIndex }) => !selection.includes(lineIndex)
    );
  }
);
