...
export const getStretchPositions = (
  glyphLine: GlyphLine
): StretchPositions | null => {
...
export const setStretchPositions = (
  glyphLine: GlyphLine,
...
export const normalizeStretchPositions = (
  positions: StretchPositions
): StretchPositions => {
...
export const calcStretchPositions = (
  param: StretchParam,
  k: number
): StretchPositions => {
...
const clampStretchScalar = (k: number): number =>
  Math.max(-10, Math.min(10, k));

export const calcStretchScalar = (
  param: StretchParam,
  positions: StretchPositions
): number => {
  const [x0, y0, x1, y1] = param;
