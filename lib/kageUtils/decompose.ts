// https://github.com/kurgm/kage-editor/blob/master/src/kageUtils/decompose.ts
// author: kurgm
...
export const decompose = (
  ...
  const strokesArray =
    ...
    kage.getEachStrokes(unparseGlyph(glyph));
  const box =
    // @ts-expect-error 2445
    kage.getBox(strokesArray);

  const x1 = glyphLine.value[3];
  const y1 = glyphLine.value[4];
  const x2 = glyphLine.value[5];
  const y2 = glyphLine.value[6];
  const [sx, sy, tx, ty] = normalizeStretchPositions(
    getStretchPositions(glyphLine)!
  );
  ...
  return glyph.map((oldGlyphLine) => {
    ...
    const [sx2, sy2, tx2, ty2] = normalizeStretchPositions(
      getStretchPositions(newGlyphLine)!
    );
...
export const decomposeDeep = (
  ...
  const strokesArray =
    // @ts-expect-error 2445
    kage.getEachStrokes(unparseGlyphLine(glyphLine));
...
export const decomposeDeepGlyph = (
  glyph: Glyph,
  buhinMap: Map<string, string>
): GlyphLine[] => {
  return glyph
    .map((glyphLine) => decomposeDeep(glyphLine, buhinMap))
