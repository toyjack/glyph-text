// https://raw.githubusercontent.com/kurgm/kage-editor/master/src/kageUtils/transform.ts
// author: kurgm
...
import { BBX } from "./bbx";
import {
  listupConnectedPoints,
  listupConnectedPointsOfSelection,
} from "./connection";

export const applyGlyphLineOperation = (
  glyphLine: GlyphLine,
  tX: (x: number) => number,
  tY: (y: number) => number
): GlyphLine => {
...
export const applyGlyphPointOperation = (
  glyph: Glyph,
  pdescs: PointDescriptor[],
  tX: (x: number) => number,
  tY: (y: number) => number
): Glyph => {
  if (pdescs.length === 0) {
...
export const moveSelectedGlyphLines = (
  glyph: Glyph,
  selection: number[],
  dx: number,
  dy: number
  ...
  glyph = glyph.map((glyphLine, index) =>
...
export const moveSelectedPoint = (
  glyph: Glyph,
  selection: number[],
  pointIndex: number,
  dx: number,
  ...
  const targetDescs = listupConnectedPoints(glyph, [selectedDesc]).filter(
    (targetDesc:{lineIndex:number}) => targetDesc.lineIndex !== lineIndex
  );
...
export const resizeGlyphLine = (
  ...
  const tX = (x: number) =>
    Math.round(x21 + ((x - x11) * (x22 - x21)) / (x12 - x11));
  const tY = (y: number) =>
    Math.round(y21 + ((y - y11) * (y22 - y21)) / (y12 - y11));
  return applyGlyphLineOperation(glyphLine, tX, tY);
};

export const resizeSelectedGlyphLines = (
  glyph: Glyph,
  selection: number[],
  oldBBX: BBX,
  newBBX: BBX
): Glyph => {
  return glyph.map((glyphLine, index) =>
    selection.includes(index)
