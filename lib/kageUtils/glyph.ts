...
export const unparseGlyphLine = (glyphLine: GlyphLine): string => {
  const values: (number | string)[] = glyphLine.value.map((num) =>
    Math.round(num)
  );
  if (values[0] === 99) {
...
export const isValidGlyphLine = (glyphLine: GlyphLine): boolean =>
  ...
  (glyphLine.value[0] !== 0 ||
    glyphLine.value[1] === 97 ||
    glyphLine.value[1] === 98 ||
    glyphLine.value[1] === 99);
...
export const unparseGlyph = (glyph: Glyph): string =>
  glyph.map((gLine) => unparseGlyphLine(gLine)).join("$");
